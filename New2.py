...
username = "WS_2005_SR"  # Replace with your Oracle username
password = "Dataentrega@2024"  # Replace with your Oracle password
...
if st.sidebar.button("Fetch Data and Upload to Google Drive"):
    try:
        # Create a temporary directory for the TNS configuration
        with tempfile.TemporaryDirectory() as temp_dir:
            tns_config_path = os.path.join(temp_dir, "tnsnames.ora")

            # Write the TNS configuration dynamically to the temporary file
            tns_config = """
WS_2005_SR =
  (DESCRIPTION =
...
"""
            with open(tns_config_path, "w") as f:
                f.write(tns_config)

            # Set the TNS_ADMIN environment variable to the temporary directory
            os.environ["TNS_ADMIN"] = temp_dir

            # Establish oracledb connection to Oracle database
            conn = oracledb.connect(user=username, password=password, dsn="WS_2005_SR")
