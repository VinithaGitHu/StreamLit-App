...
# Access Oracle credentials via user input
dsn = st.text_input("Enter Oracle DSN", value="(description=(retry_count=20)(retry_delay=3)(address=(protocol=tcps)(port=1522)(host=adb.ap-mumbai-1.oraclecloud.com))(connect_data=(service_name=g10916f2e32ac91_dataentrega_high.adb.oraclecloud.com))(security=(ssl_server_dn_match=yes)))")
    ...
    try:
        ...
        for csv_file in csv_files:
            try:
                ...
                if response.status_code == 200:
                    st.success(f"{csv_file} uploaded successfully to Migration A folder.")
                else:
